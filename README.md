...
- `workforce` 与 runtime provider 设置现在都会暴露统一只读 `role_templates_catalog`：固定包含当前 live 执行角色、未来执行角色预留、治理角色预留、文档类型和模板片段；其中 `backend / database / platform / architect / cto` 现在都已进入 CEO 招聘主链，`architect / cto` 也已进入 CEO 治理文档建票入口，但 `backend / database / platform` 仍未进入 direct CEO create-ticket，五类新增角色也都还未进入 formal runtime live 路径
