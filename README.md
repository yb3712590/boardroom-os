...
- in-process runtime 现在也支持一个最小真实 provider 适配层：当 lease owner 对应 employee 的 `provider_id=prov_openai_compat`，并且本地配置了兼容 OpenAI `responses` 的 `base_url / api_key / model` 后，runtime 会直接调用 `POST {base_url}/responses`；未配置时继续走本地 deterministic runtime，保证零配置链路不断
