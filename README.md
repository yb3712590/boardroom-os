...
- 继续把 React Boardroom UI 从“最薄可用壳”推进到完整 MVP 读面
  ...
  - 当前更明显的后续缺口，已经变成首个 scope review 通过后，还没有把 `consensus_document.followup_tickets` 继续接成下一段执行票
...
前端会通过 Vite dev proxy 直接连本地 FastAPI。当前默认 `project-init` 会尝试把 workflow 同步推进到首个 scope review；如果本地没有可派单员工，或中途出现 incident，它会停在真实 pending / incident 状态，而不是假装已经生成待审项。
