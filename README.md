...
- artifact 生命周期现在已经补到“可过期 + 默认留存 TTL + 历史临时件回填 + 调度自动清理 + dashboard / cleanup 候选读面可见状态”
...
- artifact 运维：`EPHEMERAL` 产物现在默认会带留存 TTL，历史上没写 TTL 的临时件也会被回填进 cleanup 闭环；`dashboard` 和 cleanup 候选读面可直接看到最近一次 cleanup、当前积压，以及每个 artifact 的留存来源
...
- 最近一批完成的是“多租户 worker 运维面 -> 租户级读面 + 安全批量止血入口 + 受信入口边界 + 独立操作审计读面”，以及紧接着补上的“artifact 自动清理闭环 -> 物理删除记账 + 调度自动 cleanup + 默认留存 TTL / 历史回填 / dashboard 与 cleanup 候选读面”
...
- 大文件上传还没有 multipart / 分片 / 对象存储链路，当前更适合中等体量文件；artifact 留存治理仍基于现有本地 artifact store，没有扩到大文件链路
