...
- 继续把 React Boardroom UI 从“最薄可用壳”推进到完整 MVP 读面
  - 当前已经接通 `dashboard / inbox / review room`，并在没有 active workflow 时提供最小 `project-init` 入口
  ...
  - `project-init` 目前仍只负责创建 workflow；如果要从零直接自动推进到首个 review，还需要后续后端主链补齐
...
前端会通过 Vite dev proxy 直接连本地 FastAPI。当前如果你只是刚做完 `project-init`，首页会先看到已启动 workflow；是否很快出现待审项，仍取决于现有后端链路是否已经生成 review pack。
