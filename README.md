...
- 首个 scope review 被 `board-approve` 通过后，后端现在会从已批准的 `consensus_document` artifact 里读取全部“当前支持范围内”的 follow-up，先整组校验，再按列表顺序原子创建真实执行票，并继续同步推进到下一个真实停点：新的 review、incident、无可派单员工或没有进一步状态变化
- 这条“推进到下一个真实停点”的规则现在也会复用到后续 `board-approve`：只要同一 workflow 里还有没有依赖阻塞、也没有 open review / incident 卡住的待执行票，后端就会继续消费；撞到新的治理停点才停，不再需要人工再补一次调度
- 这条 scope follow-up 续跑现在已经能把默认小项目链路推进成 `BUILD -> CHECK -> REVIEW`：`BUILD` 会先产出 `implementation_bundle@1`，再走同节点下的内部 `maker -> checker -> fix / incident` 内审，只有通过后才会放行下游 `CHECK`；`CHECK` 产出 `delivery_check_report@1`，最后再走现有 `ui_milestone_review@1 -> maker-checker -> Inbox -> Review Room` 的最终董事会审查
- 这条 scope follow-up 续跑仍然保持 fail-closed 和主线收口：当前只支持 `frontend_engineer -> ui_designer_primary`、`checker -> checker_primary`，以及 `BUILD / CHECK / REVIEW` 三种 `delivery_stage`；任一 follow-up 的 artifact、JSON、`owner_role`、`delivery_stage`、重复 `ticket_id` 或现有投影冲突不合法，整次 `board-approve` 都会被拒绝，review 继续待决，不会出现“部分落票、部分跳过”的假完成
- 多张同级 follow-up 现在也会按 ticket 隔离写入范围：每张票都会写到自己的 stage-specific 目录里，例如 `artifacts/ui/scope-followups/<ticket_id>/`、`reports/check/<ticket_id>/`、`reports/review/<ticket_id>/`，避免兄弟票互相覆盖
- Maker-Checker 现在已覆盖三条真实关键产物链：视觉里程碑 `ui_milestone_review@1`、会议/范围决策文档 `consensus_document@1`，以及只服务内部治理、不进入董事会审批的实现包 `implementation_bundle@1`
...
- 仓库现在也有一个独立运行的 React Boardroom UI 壳，放在 `frontend/`
  - 首页已接通 `dashboard + inbox + workflow river + workforce + recent events + Board Gate`
  - 首页现在还能打开 `dependency inspector`，直接看当前小项目链路里每个阶段依赖谁、停在哪、该回哪个 review / incident
  - `Inbox` 里的 review 项可直接打开 `Review Room`
  - `Inbox` 里的 incident 项也可直接打开 incident 详情，并触发现有恢复动作
  - UI 可直接提交 `project-init / board-approve / board-reject / modify-constraints / incident-resolve`
  - 前端只拥有本地视图状态，真实工作流状态仍只来自后端投影与事件流
- `dashboard.pipeline_summary.phases` 现在不再是空数组，而是固定五段的高层治理读面：`Intake / Plan / Build / Check / Review`
  - 这层摘要现在会把 scope consensus 归到 `Plan`、内部实现票归到 `Build`、内部检查票归到 `Check`、最终董事会 gate 归到 `Review`；它只服务首页河道，不是完整 DAG 回放
...
- 在已覆盖 `ui_milestone_review@1`、`consensus_document@1` 和 `implementation_bundle@1` 的 Maker-Checker 之上，继续扩更多关键产物，并补完剩余返工治理
- 继续把 React Boardroom UI 从“最薄可用壳”推进到完整 MVP 读面
  - 当前已经接通 `dashboard / inbox / review room / incident detail / workforce / recent events`，并在没有 active workflow 时提供最小 `project-init` 入口；这个入口现在会直接尝试推进到首个 scope review
  - 当前已经补上 `dependency inspector`，董事会可以直接从首页拆开看 `Plan -> Build -> Check -> Review` 当前依赖链和停点
  - 当前首页河道依赖 `dashboard.pipeline_summary.phases` 的固定五段高层摘要，不是完整 DAG 回放
  - 当前仍缺 `provider / model` 设置页
  - `workforce` 面板现在也会直接显示 `rework loops`，能看见 build 链是否卡在返工；但首页仍没有更细的返工趋势分析
  - 当前更明显的后续缺口，已经变成最终 review 之后还缺更丰富的交付 / 发布票型，以及 `delivery_check_report@1` 等更多关键产物族，而不再是 scope 之后完全没有内部 `build / check` 段
...
前端会通过 Vite dev proxy 直接连本地 FastAPI。当前默认 `project-init` 会尝试把 workflow 同步推进到首个 scope review；scope 被批准后，后端会继续把默认小项目链路同步推进成“`BUILD` 先过内部 maker-checker 内审，再进 `CHECK -> 最终 REVIEW`”，直到撞上新的 board review、incident 或无可派单员工才停。如果本地没有可派单员工，或中途出现 incident，它会停在真实 pending / incident 状态，而不是假装链路已经走完。
