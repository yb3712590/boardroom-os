...
- 命令面：`project-init`、ticket 生命周期命令、board 审批命令、`incident-resolve`
- 投影面：`dashboard`、`inbox`、`incident detail`、`review room`、developer inspector companion
- 运行时治理：重复失败升级、provider 级暂停恢复、协作式取消、`ticket-result-submit` 统一结果入口
...
当前仍有一个已知现实：全新环境下 `pip install -e .[dev]` 可能因为 `backend/` 的平铺布局触发 `setuptools` 打包识别问题，本轮没有修改这一点。
