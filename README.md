...
- 这条 scope follow-up 续跑现在已经能把默认小项目链路推进成 `BUILD -> CHECK -> REVIEW`：内部 build 产出 `implementation_bundle@1`，内部 check 产出 `delivery_check_report@1`，最后再走现有 `ui_milestone_review@1 -> maker-checker -> Inbox -> Review Room` 的最终董事会审查
...
- 继续把 React Boardroom UI 从“最薄可用壳”推进到完整 MVP 读面
  ...
  - 当前更明显的后续缺口，已经变成最终 review 之后还缺更丰富的交付 / 发布票型，以及更多关键产物族，而不再是 scope 之后完全没有内部 `build / check` 段
...
前端会通过 Vite dev proxy 直接连本地 FastAPI。当前默认 `project-init` 会尝试把 workflow 同步推进到首个 scope review；scope 被批准后，后端会继续把默认小项目链路同步推进成 `BUILD -> CHECK -> 最终 REVIEW`，直到撞上新的 board review、incident 或无可派单员工才停。如果本地没有可派单员工，或中途出现 incident，它会停在真实 pending / incident 状态，而不是假装链路已经走完。
