...
- 首个 scope review 被 `board-approve` 通过后，后端现在会从已批准的 `consensus_document` artifact 里读取第一张 follow-up，原子创建真实执行票，并继续同步推进到下一个真实停点：新的 review、incident、无可派单员工或没有进一步状态变化
...
- 继续把 React Boardroom UI 从“最薄可用壳”推进到完整 MVP 读面
  ...
  - 当前更明显的后续缺口，已经变成首个 scope review 通过后，虽然第一张 follow-up 能继续推进，但其余 follow-up 还没有落成真实票，也还没有更丰富的下游票型
...
前端会通过 Vite dev proxy 直接连本地 FastAPI。当前默认 `project-init` 会尝试把 workflow 同步推进到首个 scope review；首个 scope review 通过后，后端还会继续把第一张已批准的 follow-up 往下推到下一个治理停点。如果本地没有可派单员工，或中途出现 incident，它会停在真实 pending / incident 状态，而不是假装链路已经走完。
