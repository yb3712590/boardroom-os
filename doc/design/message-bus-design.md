...
- The trusted `worker-admin` HTTP surface now enforces a signed entry boundary: every request must carry `X-Boardroom-Operator-Token`, `platform_admin` keeps global read/write, `scope_admin` and `scope_viewer` are limited to one exact `tenant_id/workspace_id` scope, and legacy `X-Boardroom-Operator-*` headers are now only compatibility assertions rather than identity truth.
