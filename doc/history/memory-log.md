...
- 本轮 fresh 验证已通过：`python -m py_compile backend/app/contracts/advisory.py backend/app/contracts/commands.py backend/app/contracts/ceo.py backend/app/contracts/process_assets.py backend/app/core/board_advisory.py backend/app/core/approval_handlers.py backend/app/core/ceo_snapshot.py backend/app/core/ceo_prompts.py backend/app/core/process_assets.py backend/app/core/projections.py backend/app/core/constants.py backend/app/db/repository.py backend/app/db/schema.py backend/tests/test_api.py backend/tests/test_ceo_scheduler.py`、`./backend/.venv/Scripts/python.exe -m pytest backend/tests/test_api.py -k "review_room_route_returns_existing_projection or review_room_route_includes_board_advisory_context or modify_constraints or board_approve_dismisses_linked_board_advisory_session" -q`、`./backend/.venv/Scripts/python.exe -m pytest backend/tests/test_ceo_scheduler.py -k "latest_board_advisory_decision or prompt_mentions_latest_board_advisory_decision" -q`、`cd frontend && npm run test:run -- src/test/__tests__/components/ReviewRoomDrawer.test.tsx src/test/__tests__/components/ReviewRoomDrawer.persistence.test.tsx`、`cd frontend && npm run build`
- 下一轮从 `P4-S4` 起手：先补 `ProjectMap / FailureFingerprint / GraphHealthReport` 的最小合同和 advisory / process asset 血缘接线，再决定 graph patch engine 和图健康 incident 谁先落
