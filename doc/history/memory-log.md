...
- advisory change flow 当前还没把 `audit_mode = FULL_TIMELINE` 的内部 transcript 物化到 `90-archive/transcripts`；这轮先保留 session working turns、proposal / patch 资产和 apply 事件，不把这层写成已完成
- advisory analysis 当前先走最小 deterministic compiler，还没把 CEO / 架构侧内部讨论收成独立运行链和 recovery 动作；下一轮如果继续扩 P4，优先补这条
...
- 下一轮如果继续推进新架构重构，优先补 `FULL_TIMELINE` 的 advisory transcript archive，再决定是否扩更深的 architect discussion / recovery 链和第二批 `GraphHealthReport` 规则
