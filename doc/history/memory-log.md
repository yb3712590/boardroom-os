...
- advisory analysis 当前仍先走最小 deterministic compiler，还没把 CEO / 架构侧内部讨论收成独立运行链和 recovery 动作；下一轮如果继续扩 P4，优先补这条
...
- 下一轮如果继续推进新架构重构，优先补 advisory analysis 的独立运行链和 recovery 动作，再决定是否扩第二批 `GraphHealthReport` 规则
