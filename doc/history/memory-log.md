...
- `./backend/.venv/Scripts/python.exe -m pytest backend/tests/test_ceo_scheduler.py -q` 这轮额外补跑仍有一批旧 helper 失败；当前判断是旧的 `source_code_delivery / consensus_document / governance_document` 夹具没有迁到现状 deliverable contract，不算 `P2-S7 / P2-S8` blocker，留给后续 closeout / approval 历史测试桶一起收口
