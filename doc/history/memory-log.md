...
- `BoardAdvisorySession.approved_patch_ref` 这轮先指到 `DECISION_SUMMARY` 过程资产，不是独立 graph patch engine；当前先靠 advisory decision + CEO rerun 影响主链，后续 `P4-S4+` 再补 `ProjectMap / graph patch engine / GraphHealthMonitor`
...
- 下一轮如果继续推进新架构重构，优先从 `approved_patch_ref -> graph patch engine` 的独立切片起手，再决定是否扩第二批 `GraphHealthReport` 规则和更细的 `ProjectMap` 切片
