...
2. 新建 `src/boardroom_os/contracts/refs.py`，提供三类纯函数 helper：
   - `namespaced_ref(...)`：跨包引用命名空间助手（解决 P1-5），输入 `content_hash` 必须通过既有 `Sha256Hex`（SHA-256 摘要值对象）校验。
   - `canonical_sort_for_hash(...)`：集合语义输入规范排序助手（解决 P2-1/2/3）。
   - `hash_namespaced_payload(...)`：统一 canonical JSON SHA-256 payload hash（规范 JSON 负载哈希）入口。
...
- 不引入新的 SHA value object（与 V2-070G P2-13 重叠的部分按既有 `boardroom_os/contracts/hashes.py` 中的 `Sha1Hex` / `Sha256Hex` 复用；V2-071A 只新增 namespace + canonical sort + payload hash helper）。
...
其中 `content_hash_short` 必须由完整 `Sha256Hex`（SHA-256 摘要值对象）的前 12 位派生，不能由调用方直接传短 hash。

例如：

- `replay-bundle.project.abc123def456.<run_id_short>`（旧实现：`replay-bundle.<project>.<summary_hash[:12]>`，缺 run_id）
- `process-audit-artifact.project.abc123def456.run-001.timeline`（旧实现：`process-audit-artifact.timeline`，完全无 project / content）
- `git-version-audit-facts.project.<git_version_audit_content_hash[:12]>`（旧实现：`git-version-audit-facts.<project>`，缺 commit 内容绑定段；`git_version_audit_content_hash` 必须由包含 `base_commit_sha` / `final_commit_sha` 的 Git facts payload canonical hash 派生，不能直接使用 40 位 Git commit SHA-1）

`extra_suffix` 只能在 `run_id` 存在时追加，禁止只传 `extra_suffix` 形成单尾段引用；因此 4 段引用的尾段一律解释为 `run_id`，5 段引用才包含 `extra_suffix`，便于后续反向解析或 lineage（血缘关系）分组。

namespace 段的非空字符必须满足 `_NAMESPACE_SEGMENT_PATTERN = r"^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$"`（小写字母数字与连字符，不能以连字符开头或结尾）。
...
def namespaced_ref(
    *,
    ...
    if isinstance(content_hash, Sha256Hex):
        digest = content_hash.value
    else:
        digest = Sha256Hex(value=content_hash).value
...
def hash_namespaced_payload(payload: dict[str, Any]) -> str:
    ...
    复用各模块内 `_hash_jsonable` 的 canonicalization 语义，支持 BaseModel、StrEnum、tuple/list 与 nested Mapping。
    """
...
- `extra_suffix` 必须与 `run_id` 同时出现；禁止只追加 `extra_suffix`，避免 4 段 ref 的尾段在反向解析时同时可能代表 `run_id` 或 suffix。
- `content_hash` 必须是完整 lowercase SHA-256 hex digest（小写十六进制 SHA-256 摘要），通过 `Sha256Hex` 校验并拒绝 placeholder / synthetic digest（占位/合成摘要）；输出引用只使用前 12 位作为短段。
- 拼接顺序固定，便于反向解析。
...
- `_canonical_jsonable` 先递归规范化 `BaseModel`、`StrEnum`、tuple/list 与 nested `Mapping`，与 V2-070 现有 `_hash_jsonable` 语义一致。
- `sort_keys=True` 保证字段顺序不影响 hash。
- `ensure_ascii=False` 保留 UTF-8 内容。
- 非 dict payload 或不可 JSON 序列化内容必须 raise `NamespacedRefError`。
...
4. `test_namespaced_ref_rejects_short_content_hash` — `content_hash="abc"` 必须 raise（由 `Sha256Hex` 拒绝）。
...
10. `test_namespaced_ref_rejects_placeholder_content_hash` — `content_hash="a" * 64` 必须 raise（复用 `Sha256Hex` 占位摘要校验）。
11. `test_namespaced_ref_rejects_extra_suffix_without_run_id` — 只传 `extra_suffix` 不传 `run_id` 必须 raise，避免单尾段反向解析歧义。
12. `test_hash_namespaced_payload_rejects_non_dict_payload` — 顶层 payload 非 dict 必须 raise。
13. `test_hash_namespaced_payload_rejects_non_jsonable_payload` — 不可 JSON 序列化 payload 必须 raise `NamespacedRefError`。
...
7. `test_hash_namespaced_payload_canonicalizes_models_enums_and_sequences` — `BaseModel`、`StrEnum`、tuple/list 与 nested mapping 先规范化再 hash，与等价 JSON dict/list 产生相同 hash。
