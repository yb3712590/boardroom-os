...
| Phase 10：Agent-team rework loop hardening | V2-100 | 6 / 6 | V2-100A~F 完成 |
| **合计** | **V2-000 ~ V2-100** | **75 / 76** | **V2-090A ~ V2-090E、V2-090G、V2-090H、V2-090I、V2-090J、V2-090K 与 V2-100A/B/C/D/E/F 完成；V2-090F REVIEW_REQUIRED / BLOCKED** |
...
- 输入文档：`docs/superpowers/specs/2026-06-12-v2-090f-atomic-golden-sample-rebuild-design.md`、`docs/superpowers/plans/2026-06-12-v2-090f-atomic-golden-sample-rebuild.md`、V2-090A~K、V2-100A~F 产物。旧 `v2-090f-rerun-rework-entry-validation-*` 文档仅保留为 historical / superseded（历史 / 已被取代）复盘材料，不再作为 active implementation entry（活跃实施入口）。
- 依赖：V2-090A~E、V2-090G、V2-090H、V2-090I、V2-090J、V2-090K、V2-100F；V2-100F 已证明 RunManifest tolerant ingestion（运行清单宽容摄取）到 provider-backed blackbox verification（模型支撑黑盒验证）和 evidence/rework routing（证据/返工路由）的原生能力，V2-090F 仍需真实 provider opt-in（真实模型显式启用）跑通新 native PRD delivery（原生需求交付）入口后才可复判 DONE。
- 输出文件：`examples/directives/tiny-fullstack-prd.md`、`examples/directives/v2-090f-reference-examples.md`、`config/boardroom-runtime.v2-090f.yaml`、`config/boardroom-providers.v2-090f.yaml`、`config/boardroom-roles.v2-090f.yaml`、`src/boardroom_os/orchestration/prd_delivery.py`、`src/boardroom_os/proving/v2_090f_native_golden_sample.py`、`src/boardroom_os/proving/v2_090f_prd_agent_team.py`（内部 helper，非完成证明入口）、`scripts/run_boardroom_prd_delivery.py`、`scripts/run_v2_090f_native_golden_sample.py`、`scripts/build_tiny_closeout_sample.py`、`examples/generated-workspaces/tiny-fullstack/`、`examples/README.md`、`scripts/README.md`、`tests/orchestration/test_prd_delivery.py`、`tests/proving/test_v2_090f_native_golden_sample.py`、`tests/proving/test_v2_090f_native_golden_sample_real.py`、`tests/negative/test_v2_090f_native_golden_sample_fail_closed.py`。
...
- 当前阻塞：2026-06-12 已修订 spec/plan 并自审，明确废弃旧 provider-backed generation subprocess、provider artifact lock 和固定三票监督实施作为成功路径。第一次真实 provider run（真实模型运行）跑通 worker implementation chain（工人实施链路）和基础 CRUD，但专家评审认定存在明确外部介入。2026-06-13 V2-090K 已移除这些自治偏移并保留结构化 fail-closed 现场；2026-06-22 V2-100F 已证明原生 RunManifest tolerant ingestion（运行清单宽容摄取）→ verify-blackbox TicketGraph（黑盒验证工单图）→ ExecutionPackage（执行包）→ provider-backed BlackboxVerificationPlan（模型支撑黑盒验证计划）→ runner facts（运行事实）→ evidence/rework routing（证据/返工路由）链路。2026-06-23 旧 `scripts/run_v2_090f_prd_agent_team.py --stage rework-entry` 与 `src/boardroom_os/proving/v2_090f_rework_entry.py` 已删除；V2-090F 当前 active path（活跃路径）改为 `run_prd_delivery()` + `run_v2_090f_native_golden_sample()`。V2-090F 仍保持 REVIEW_REQUIRED / BLOCKED，等待真实 provider opt-in 证明和专家验收。
...
- 当前证据：2026-06-16 真实 V2-090F rerun 在 RunManifest assertion normalization（运行清单断言归一化）阶段因 `json_array_contains_field` raw crash，导出 `blocked_by_missing_rework_entry`；样例工程后端 CRUD 和本地测试可运行，但 RunManifest 与实现存在 readiness path（就绪路径）和 response shape（响应结构）等真实 drift（漂移）。2026-06-21 已丢弃偏外接 helper/hook（辅助器/钩子）的旧 plan，重写为 V2-100F-A~F。V2-100F-A 已完成宽容摄取上下文；V2-100F-B 已证明 `verify-blackbox` 通过 `TICKET_CREATED -> SEAT_ASSIGNED -> SeatAssignmentGraph.ready_queue -> ExecutionPackageCompiler` 原生链路进入执行包；V2-100F-C 已完成 provider-backed `BlackboxVerificationPlan`（模型支撑黑盒验证计划）schema（结构）、`BlackboxPlanApproval`（黑盒计划批准）动作门禁与 lineage validator（来源链校验器）；V2-100F-D 已完成 `BlackboxPlanRunner`（黑盒计划运行器）和 `BlackboxActionExecutionFact`（黑盒动作执行事实）；V2-100F-E 已完成 `RUN_MANIFEST_ERROR`（运行清单错误）active routing code（活跃路由代码）、黑盒 fact 到 `LiveBlackboxIntegrationEvidence`（真实黑盒集成证据）的薄适配和 `REWORK_REQUESTED -> ReworkReducer`（返工请求事件到返工归约器）链路。V2-100F-F 已完成 thin proving harness（薄证明驱动器）和真实 provider opt-in 证明：base `.env` 仅在测试命令作用域读取，显式覆盖到 `config/boardroom-runtime.v2-090f.yaml`、`config/boardroom-providers.v2-090f.yaml`、`config/boardroom-roles.v2-090f.yaml`；`tests/proving/test_v2_100f_real_provider_orchestration.py -q -s` 通过（1 passed in 27.04s），终态属于三态之一且无 `unsupported RunManifest behavior assertion type`。配套验证：focused V2-100F suite 通过（25 passed, 1 skipped），wide regression `tests/reducers tests/execution tests/evidence tests/closeout tests/rework tests/negative -q` 通过（1566 passed），`RUN_MANIFEST_MISMATCH` active guard 无匹配，`git diff --check` 通过。AC-V2-REWORK-006 已勾选；V2-090F golden sample 仍保持 REVIEW_REQUIRED / BLOCKED，等待后续评审或重跑。
