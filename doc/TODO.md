...
- `P0-COR-002` 进行中：收正单一 workflow controller。当前第三段已落：新增 `workflow_progression` shared abstraction，`project-init` kickoff、requirement elicitation 后续 kickoff、`workflow_controller / ceo_scheduler / deterministic fallback / validator / workflow_auto_advance` 现在都开始共用同一套 adapter 真相；`CEO_AUTOPILOT_FINE_GRAINED` 已切 governance-first，controller 会在治理链未走完时暴露 `GOVERNANCE_REQUIRED`
- `P0-COR-003` 进行中：收正 capability gap 驱动的招聘、协作和阻断逻辑。当前第三段已落：治理链走到 backlog recommendation 后，autopilot fanout 才会进入 `architect_primary / 技术决策会议 / staffing gap` 这些硬约束；`required_governance_ticket_plan` 也已从“只补 architect brief”扩到通用“下一张治理文档票”。当前仍未完成的是：`STANDARD` 还挂在 `STANDARD_LEGACY_SCOPE_CHAIN` adapter 上，具体迁到 governance-first 已明确留给下一轮优先处理
