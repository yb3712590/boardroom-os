...
- 当前 `P2-S2` 已完成：workspace-managed 源码票现在已经走正式 `RoleHook` registry、required hook gate、`REQUIRED_HOOK_GATE_BLOCKED` incident 和 `REPLAY_REQUIRED_HOOKS` recovery；下一轮转 `P2-S3`，把同一套 hook gate / replay 协议扩到下一类真实主线票型，并评估 project-init governance/provider 老 incident 噪音的收口顺序
