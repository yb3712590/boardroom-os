...
- 下一轮如果继续推进 `P4-S4` 收尾瘦身，优先决定 `GraphHealth` 的 queue / stale / cross-version 这批 runtime-liveness 规则是否后移出 graph 内核；placeholder runtime materialization 和 runtime `node_projection` 双层真相继续保持后置单独决策
...
- 下一轮如果继续推进新架构重构，继续从 `P4-S4` 续跑，优先决定 placeholder node 的 runtime materialization 是否拆独立切片；graph layer identity 与 graph-only add-node 已收口，runtime `node_projection` 是否继续拆层保持后置单独决策；`doc/new-architecture/**` 仍保持只读
