...
- `HIRE_EMPLOYEE` 当前已分成双轨：自动主线里的 CEO 动作会直接注册 active employee；手工 `employee-hire-request` 仍继续走 `CORE_HIRE_APPROVAL`
...
- 2026-04-20 `runtime ticket / legacy compat` 硬切收尾这轮已把当前主链收口：`test_api.py` 里 `scope review -> final review -> closeout` 历史 helper 已贴住新主线，`test_scheduler_runner.py` 里 provider-backed / timeout / repeated-failure 三条 recovery 历史桶也已能走到 closeout 票完成；`ceo_execution_presets.py` 与 `test_ceo_scheduler.py` 里的旧 consensus follow-up 文案和 helper 也已清掉
- 2026-04-20 同轮继续把 minimal recovery seed 的 graph health / workflow completion truth 收口：timeout / repeated-failure 两条历史桶现在不止是 closeout 票 `COMPLETED`，workflow projection 也会稳定落到 `COMPLETED`，dashboard `completion_summary` 会回填，额外误开的 `GRAPH_HEALTH_CRITICAL` 也已消失；这轮同时把跨 lane parent 解析、`ORPHAN_SUBGRAPH` 的 `REVIEWS` 收敛口径、final review 后的同步 closeout auto-advance，以及 closeout completion 对 stale lineage overlap 的判定一起收正
...
- 2026-04-19 本轮继续收紧 `P2-S7` 的 CEO action contract：live provider 入口现在只保留 `type -> action_type` 这条纯语法 alias，旧 `NO_ACTION.reason`、旧 `HIRE_EMPLOYEE` 扁平字段和 flat `CREATE_TICKET` 结构都已显式拒绝；deterministic fallback 若命中 `CREATE_TICKET / HIRE_EMPLOYEE / REQUEST_MEETING` 但 capability plan 结构残缺，也会直接在 proposal 阶段抛错并继续复用 `CEO_SHADOW_PIPELINE_FAILED -> RERUN_CEO_SHADOW`，不再静默落成 `NO_ACTION`
- 2026-04-19 本轮已删掉 `ceo_proposer` 里一批只服务旧 provider payload 语义补全的 compat helper；当前 CEO 主链只再接受完整 `ceo_action_batch_v1`，不再靠 snapshot 反向补 `execution_contract / dispatch_intent / dependency_gate_refs / parent_ticket_id`
- 2026-04-19 本轮 fresh 验证已通过：`py -m py_compile backend/app/core/ceo_proposer.py backend/tests/test_ceo_scheduler.py`、`py -m pytest backend/tests/test_ceo_scheduler.py -k "records_fallback_without_touching_mainline_state or pipeline_failed_raises_without_hidden_fallback or opens_ceo_shadow_pipeline_failed_incident or project_init_records_board_directive_shadow_and_stable_scope_ticket or hires_architect_before_backlog_followup_when_required or creates_architect_governance_ticket_before_backlog_followup_when_doc_is_missing or requests_meeting_before_backlog_followup_when_required or flat_create_ticket_action_shape or live_action_type_alias_field or explicit_governance_dependency_chain_and_process_assets or legacy_no_action_reason_shape or legacy_hire_employee_payload_shapes or deterministic_project_init_kickoff_has_no_resolved_assignee or deterministic_required_governance_plan_is_incomplete or deterministic_hire_plan_is_incomplete or deterministic_backlog_followup_plan_is_incomplete or deterministic_meeting_request_has_no_eligible_candidate" -q`、`py -m pytest backend/tests/test_workflow_autopilot.py -k "ceo_shadow_pipeline_incident" -q`、`py -m pytest backend/tests/test_api.py -k "ceo_shadow_incident_detail or ceo_shadow_incident_resolve_should_rerun_and_close or placeholder_gate_incident_resolve_reruns_shadow_and_closes_incident" -q`
- 2026-04-22 本轮已把这条硬切继续收完：`required_governance_ticket_plan` 现已正式收成 `existing_ticket_id + ticket_payload`，`followup_ticket_plans[]` 现已正式收成 `ticket_key + existing_ticket_id + blocked_by_plan_keys + ticket_payload`；controller / proposer / validator 都只再消费 canonical `ticket_payload`
- 2026-04-22 OpenAI compat CEO shadow 调用现已显式带 `ceo_action_batch` strict schema，live provider 回流旧版 `execution_contract`、漏 `dispatch_intent.selection_reason`、继续用 top-level legacy 字段时，现在都会在 proposal 阶段直接 fail-closed，不再做 compat 补齐
- 2026-04-22 同轮补了一条调度侧真相：proposal 失败但 provider 已回包时，CEO shadow run audit 现在会保留 `provider_response_id`；idle maintenance 会忽略首轮 `BOARD_DIRECTIVE_RECEIVED` 打开的 `CEO_SHADOW_PIPELINE_FAILED`，允许 project-init 后继续按周期重试
- 2026-04-22 本轮 fresh 验证已通过：`pytest backend/tests/test_ceo_scheduler.py -q`（`108 passed`）、`pytest backend/tests/test_execution_targets.py -q`（`1 passed`）
- 2026-04-22 当前还留一条非 CEO shadow 主线回归待单独收口：`pytest backend/tests/test_api.py -k "ticket_create_rejects_missing_execution_contract or scheduler_tick_dispatches_to_explicit_assignee_from_dispatch_intent or ticket_create_is_rejected_when_dispatch_intent_dependency_gate_is_invalid" -q` 中 `test_scheduler_tick_dispatches_to_explicit_assignee_from_dispatch_intent` 仍失败，现象是 `scheduler-tick` 后 ticket 停在 `PENDING`
...
- `P0-COR-006` 进行中：当前已把 live runner 抽成共享 harness；固定入口保留 `backend/tests/live/requirement_elicitation_autopilot_live.py`、`backend/tests/live/architecture_governance_autopilot_live.py`，图书馆 full live 已改成 `backend/tests/live/run_configured.py --config backend/data/live-tests/library_management_autopilot_live.toml`。这轮新增了 `backend/tests/live/architecture_governance_autopilot_smoke.py` checkpoint smoke，用真实 provider 验“招聘架构师 + 技术决策会议 + 架构治理文档批准”；但这台机器上实跑仍被 provider timeout 卡住，`requirement_elicitation` 和 `library_management` 的 full real-provider 长测也仍待重跑
...
- 2026-04-18 本轮已继续落 `integration-test-002` 剩余修复：in-process runtime 现在会同时处理 `LEASED` 和 `EXECUTING`，后者按同一条 compile -> execute -> result-submit 链做 idempotent resume，不再重复 `ticket-start`
- 2026-04-18 review lane 的 compiled execution package guard 这轮已收正到显式 graph truth：`ticket-result-submit` 现在会按当前 ticket 的 `graph_contract.lane_kind` 解析真实 `graph_node_id`，review lane 不会再误读 execution lane 的 runtime node version
- 2026-04-18 本轮已补齐 scheduler/live harness 的 runtime 审计摘要：`SCHEDULER_ORCHESTRATION_RECORDED.payload.runtime_execution` 现在会写 `action/start_ack_status/final_ack_status/skipped[]`；`run_report.json`、`audit-summary.md`、`integration-monitor-report.md` 和 failure snapshot 也会同步带 `runtime_execution_summary`
- 2026-04-18 这轮把 runtime / compiler / ticket result guard / runtime-node projection rebuild 的 lane 读取收成 `contract-only`；旧的 `apply_legacy_graph_contract_compat()` 不再参与这条 active mainline 真相链，坏 contract 会 fail-closed，不再隐式猜 lane
- 2026-04-18 本轮 fresh 验证已通过：`py -m py_compile backend/app/core/context_compiler.py backend/app/core/reducer.py backend/app/core/runtime.py backend/app/core/ticket_handlers.py backend/app/db/repository.py backend/app/scheduler_runner.py backend/tests/live/_autopilot_live_harness.py backend/tests/test_api.py backend/tests/test_live_library_management_runner.py backend/tests/test_scheduler_runner.py`、`py -m pytest backend/tests/test_scheduler_runner.py::test_runtime_runner_executes_leased_review_lane_ticket backend/tests/test_scheduler_runner.py::test_runtime_runner_resumes_executing_ticket_without_restarting backend/tests/test_scheduler_runner.py::test_scheduler_runner_records_orchestration_trace_in_execution_order backend/tests/test_scheduler_runner.py::test_scheduler_runner_records_runtime_skip_reason_for_missing_graph_contract backend/tests/test_api.py::test_ticket_start_rejects_stale_runtime_node_projection_version backend/tests/test_api.py::test_ticket_start_rejects_stale_review_lane_runtime_node_projection_version backend/tests/test_api.py::test_ticket_result_submit_rejects_stale_runtime_node_projection_version backend/tests/test_api.py::test_ticket_result_submit_accepts_review_lane_package_when_execution_lane_version_drifts backend/tests/test_api.py::test_ticket_result_submit_rejects_stale_review_lane_runtime_node_projection_version backend/tests/test_live_library_management_runner.py::test_build_audit_snapshot_uses_current_workflow_runtime_execution_summary backend/tests/test_live_library_management_runner.py::test_write_audit_summary_renders_runtime_execution_summary backend/tests/test_live_library_management_runner.py::test_write_integration_monitor_report_renders_runtime_execution_summary backend/tests/test_live_library_management_runner.py::test_write_failure_report_persists_run_report_for_failed_snapshot --basetemp backend/.tmp/pytest-final-verify -q`
...
- `P2-RLS-002` 已完成（2026-04-08，2026-04-21 补主线语义）：CEO `HIRE_EMPLOYEE` 现在已放宽到 `backend / database / platform / architect / cto` 五类新增角色；自动主线里的 CEO 招聘现已直接注册 active employee，不再经过 `CORE_HIRE_APPROVAL`。手工 `employee-hire-request` 仍保留审批链；`architect_primary / cto_primary` 已进入 CEO 治理文档建票入口，并通过最小 `execution_contract + legacy role_profile:*` 兼容路径执行；`backend / database / platform` 已进入 meeting participant 匹配与 `BUILD` follow-up owner_role；`CHECK` 仍只给 `checker`，`REVIEW` 仍只给 `frontend_engineer`
...
### 2026-04-25??????? decomposition

- ??????????????? ticket ?? `REQUEST_TOO_LARGE / CONTEXT_TOO_LARGE / OUTPUT_TOO_LARGE / NEEDS_DECOMPOSITION` ?????? retry ???? ticket?????? `retry_budget`?
- ??????? `DECOMPOSITION_RECOVERY` incident???? `decomposition_recovery` ????source ticket/node/schema?failure fingerprint?recovery policy?decomposition decision/plan?segment ticket ids ? aggregator ticket id?
- ?? deterministic recovery plan ???? `scope_and_requirements` ? `solution_and_risks` ?? `architecture_brief_segment` ?? ticket??? aggregator ?? `dependency_gate_refs` ??? segment artifact refs ?????? schema?
- ?? source created spec ???? schema/role/replayable input artifact ???????????? `BLOCKED`?? fallback?????? ticket??? provider hidden state?
- ?????`tests/test_decomposition.py`?`tests/test_api.py::test_decomposition_recovery_*` ?? plan ????? provider/fallback ?????????????????? retry?
