...
- 下一轮如果继续推进新架构重构，继续从 `P4-S7` 续跑，优先决定 review lane / approval target / worker runtime 是否继续拆成 graph-first 双层真相；`RuntimeLiveness/GraphHealth` 的 policy contract 是否继续拆层单独决策；`doc/new-architecture/**` 仍保持只读
