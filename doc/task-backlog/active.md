# Active Task Backlog

> 说明：这里只保留当前仍未关闭、仍可能被反复读取的任务。已完成的详细任务卡片和完成补记统一看 `done.md`。

## 快速定位

| 方向 | 任务范围 | 默认状态 | 备注 |
|------|----------|----------|------|
| 会议室自动触发 | `P1-MTG-008` | 未开始 | 当前会议室最小版已落地，但 CEO 还不会自动触发会议 |
| 冻结能力隔离 | `P1-CLN-001` 到 `P1-CLN-006` | 未开始 | 目前只做了文档隔离，没有做 `_frozen/` 物理迁移 |
| 检索层 | `P2-RET-001` 到 `P2-RET-005` | 未开始 | 仍属后置增强 |
| Provider 增强 | `P2-PRV-001` 到 `P2-PRV-008` | 未开始 | 仍属后置增强 |
| 治理模板与文档型角色 | `P2-GOV-001` 到 `P2-GOV-006` | 未开始 | 仍属后置增强 |
| UI 打磨 | `P2-UI-001` 到 `P2-UI-008` | 未开始 | 仍属后置打磨 |
| 文档 | `P2-DOC-001` 到 `P2-DOC-005` | 未开始 | 其中 `README / TODO / memory-log` 已有当前版本，后续按真实变更继续同步 |

## P1：重要

### 3.2 会议室协议（仍未关闭）

| ID | 标题 | 预估 | 状态 |
|----|------|------|------|
| P1-MTG-008 | CEO 触发会议决策 | 4h | 未开始 |

当前边界：

- `ticket-backed meeting room` 的最小闭环已经落地，只开放 `TECHNICAL_DECISION`
- runtime 已能执行 `POSITION -> CHALLENGE -> PROPOSAL -> CONVERGENCE` 四轮，并回到现有 maker-checker / board review
- 当前明确没有做 CEO 自动触发会议，所以 `P1-MTG-008` 仍保留在活跃页

### 3.3 代码清理

> 当前状态补记：这 6 个任务本轮没有执行。这轮只借用了它们的边界梳理思路完成文档隔离，没有做 `_frozen/` 物理迁移。

| ID | 标题 | 预估 | 状态 |
|----|------|------|------|
| P1-CLN-001 | 移动 worker-admin 代码到 _frozen/ | 3h | 未开始 |
| P1-CLN-002 | 移动多租户代码到 _frozen/ | 2h | 未开始 |
| P1-CLN-003 | 移动对象存储代码到 _frozen/ | 2h | 未开始 |
| P1-CLN-004 | 移动远程 handoff 代码到 _frozen/ | 2h | 未开始 |
| P1-CLN-005 | 更新 import 路径 | 3h | 未开始 |
| P1-CLN-006 | 标记或移除 frozen 测试 | 4h | 未开始 |

## P2：增强

### 4.1 检索层

| ID | 标题 | 预估 | 状态 |
|----|------|------|------|
| P2-RET-001 | 创建 FTS5 虚拟表 | 4h | 未开始 |
| P2-RET-002 | 索引工单结果和审查摘要 | 4h | 未开始 |
| P2-RET-003 | Context Compiler 集成 FTS5 查询 | 4h | 未开始 |
| P2-RET-004 | 检索结果排序和去重 | 4h | 未开始 |
| P2-RET-005 | 检索层测试 | 4h | 未开始 |

### 4.2 Provider 增强

| ID | 标题 | 预估 | 状态 |
|----|------|------|------|
| P2-PRV-001 | 多 Provider 配置支持 | 4h | 未开始 |
| P2-PRV-002 | 能力标签定义 | 3h | 未开始 |
| P2-PRV-003 | 基础健康检查 | 3h | 未开始 |
| P2-PRV-004 | 简单 fallback 路由 | 4h | 未开始 |
| P2-PRV-005 | Provider 增强测试 | 6h | 未开始 |
| P2-PRV-006 | 角色级默认模型绑定 | 4h | 未开始 |
| P2-PRV-007 | 任务级模型覆盖与 preferred/actual model 追踪 | 4h | 未开始 |
| P2-PRV-008 | 成本分层与高价模型低频路由 | 4h | 未开始 |

### 4.3 治理模板与文档型角色

| ID | 标题 | 预估 | 状态 |
|----|------|------|------|
| P2-GOV-001 | 定义治理模板数据结构 | 4h | 未开始 |
| P2-GOV-002 | 定义 CTO / 架构师低频角色模板 | 4h | 未开始 |
| P2-GOV-003 | 定义架构 / 选型 / 里程碑 / 详细设计 / TODO 文档产物契约 | 4h | 未开始 |
| P2-GOV-004 | CEO 按治理模板触发文档型任务 | 4h | 未开始 |
| P2-GOV-005 | 文档型角色默认不参与日常编码 / 测试执行约束 | 3h | 未开始 |
| P2-GOV-006 | 治理模板与文档型角色测试和文档 | 5h | 未开始 |

### 4.4 UI 打磨

| ID | 标题 | 预估 | 状态 |
|----|------|------|------|
| P2-UI-001 | Workflow River 粒子动画 | 4h | 未开始 |
| P2-UI-002 | Board Gate 呼吸动画优化 | 2h | 未开始 |
| P2-UI-003 | 加载骨架屏全覆盖 | 3h | 未开始 |
| P2-UI-004 | 响应式布局 | 4h | 未开始 |
| P2-UI-005 | 键盘可访问性 | 3h | 未开始 |
| P2-UI-006 | 暗色主题微调 | 2h | 未开始 |
| P2-UI-007 | 性能优化（懒加载、debounce） | 3h | 未开始 |
| P2-UI-008 | UI 打磨测试 | 3h | 未开始 |

### 4.5 文档

| ID | 标题 | 预估 | 状态 |
|----|------|------|------|
| P2-DOC-001 | 更新 README.md | 2h | 未开始 |
| P2-DOC-002 | 更新 doc/TODO.md | 2h | 未开始 |
| P2-DOC-003 | 编写运维指南 | 3h | 未开始 |
| P2-DOC-004 | 更新 memory-log.md | 2h | 未开始 |
| P2-DOC-005 | 编写 API 文档 | 3h | 未开始 |

## 依赖提醒

- `P1-MTG-008` 依赖当前会议室最小闭环已经稳定，不适合和 UI 打磨或检索层一起混做
- `P1-CLN-*` 如果真正启动，应先做依赖图和调用清单，不要直接搬目录
- `P2-RET-*`、`P2-PRV-*`、`P2-GOV-*` 目前都属于后置增强；只有在本地 MVP 主链已经证明需要时，才值得打开
- `P2-UI-*` 和 `P2-DOC-*` 更适合作为发布前收口，不要反过来影响主链真相和现有验证链
