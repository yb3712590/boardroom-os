...
- 这份时间线现在主要用来排中长期顺序，不再代表当前代码的实时落地状态。
- 当前真实状态以 `doc/mainline-truth.md` 和 `doc/TODO.md` 为准；很多 M0~M7 里的能力已经部分或全部落地。
- 只有在要排多周计划、发布顺序或远期能力优先级时，才需要继续读全文。

## 下一步关键路径

- 当前最相关的后续收口是：冻结能力边界澄清、后置增强是否真的需要、UI 与文档收口。
- 真正要打开 `检索 / Provider 路由 / 发布准备` 时，再回到本时间线确认它们在整体顺序里的位置。
- 这份文档适合做跨周排程，不适合判断“今天该做什么”。
...
Boardroom OS 当前已经跑通本地 MVP 主闭环，并落地了 CEO 有限接管首轮、真实 Worker 执行、人格模型、会议室最小版和 React 治理壳。本文现在主要用于安排剩余的中长期收口工作，不再代表每个能力的实时落地状态。

本时间线从 2026-04-07 开始，总计 13 周（至 2026-07-04），分 9 个里程碑推进。当前如果只是默认阅读，不需要先把全文读完。
