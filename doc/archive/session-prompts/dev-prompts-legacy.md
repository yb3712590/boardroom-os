...
| 唯一待办 | `doc/TODO.md` | 当前批次与 `C1` 条件批次 |
| 稳定基线 | `doc/history/context-baseline.md` | 产品模型、治理规则、架构基线 |
| 工作记忆 | `doc/history/memory-log.md` | 最近几天仍影响判断的事实 |
...
| 里程碑时间线 | `doc/milestone-timeline.md` | 后续顺序、条件批次、远期储备 |
...
5. 需要后续顺序、条件批次或远期储备时，再读 doc/milestone-timeline.md
6. 按需读相关设计文档，不读无关文件
7. doc/history/archive/* 仅在需要精确历史原因时查看

推进规则：
- 以 doc/TODO.md 的当前批次为主线；`C1` 条件批次只有在触发条件成立时才可开启
- 以 doc/task-backlog/active.md 的任务清单为执行依据；需要历史卡片时再查 doc/task-backlog/done.md
- 以 doc/milestone-timeline.md 的后续顺序为参考，而不是当前真相源
- 不因“后置增强”或“条件纳入”处于未开始状态就自动开工
- 如果当前只有 blocker 或条件未满足，要明确说明本轮无可直接开启任务，不强行挑远期任务
- 优先完成当前批次中可直接开启的最高优先级任务
- 同一闭环内的相邻环节可以连续推进，允许跨后端/前端一起做
- 不开多个无关方向，不顺手优化，不为假想需求铺扩展点
- 已降级代码（worker-admin / 多租户 / 对象存储 / 远程 handoff）默认视为冻结；只有直接服务当前批次时才允许最小解堵，不顺势扩张

开始前输出：
1. 当前所处主线状态和进度判断（一句话）
2. 本轮要完成的具体任务 ID 列表（引用 doc/task-backlog/active.md；如果没有满足开启条件的任务，要明确写出原因）
3. 每个任务完成后的直接结果
4. 本轮明确不做的内容
...
第一步：验证
- 运行后端测试：cd backend && ./.venv/bin/pytest tests/ -q
...
- 合并分支到 main，删除本地分支
- git commit 和 git push（提交信息用中文，格式：feat/fix/refactor/docs: 一句话描述）
...
```md
继续推进 Boardroom OS 前端治理壳，在现有代码上增量开发。
...
4. 需要任务 ID、状态和详细约束时，再读 doc/task-backlog.md → doc/task-backlog/active.md
5. 需要后续顺序、条件批次或远期储备时，再读 doc/milestone-timeline.md
6. doc/design/frontend-architecture-guide.md（先看 TL;DR）
7. doc/design/frontend-component-spec.md（先看 TL;DR）
8. doc/design/boardroom-ui-visual-spec.md（需要时）
9. frontend/README.md
10. 按需读当前要改的组件源码

推进规则：
- 只做当前批次内可直接开启的前端相关任务；不要因为“后置增强”或“条件纳入”处于未开始就自动开工
- 如果当前没有满足开启条件的前端任务，要明确说明原因，不强行挑远期工作
- 功能必须与当前治理壳语义一致，不凭感觉加新功能
- 样式遵循前端架构指南中的设计令牌和命名规范
- 组件 props 接口遵循组件规格说明
- 同一前端闭环内的相邻环节可以连续推进，但不要重开已关闭的大方向
...
实现要求：
- 交付可运行前端代码 + 最小测试 + 必要文档更新
- 如果本轮触及共享契约、接口或后端数据形状，要同步做最小后端验证
- 不把 stub / placeholder 写成已完成
...
- 运行前端构建：cd frontend && npm run build
- 运行前端测试：cd frontend && npm run test:run
- 如果触及共享契约、接口或后端返回形状，再运行：cd backend && ./.venv/bin/pytest tests/ -q
- 如果任何一项失败，先修复再继续
...
2. 落地了哪些能力
3. 哪些仍是 mock / stub
4. 测试结果（前端 build 状态 / 前端 passed 数；如有后端验证也说明）
5. 更新了哪些文档
6. git commit hash
7. 下一步最合理的任务 ID
8. 当前里程碑完成度，整个项目的完成度
...
4. 需要任务 ID、状态和详细约束时，再读 doc/task-backlog.md → doc/task-backlog/active.md
5. 需要后续顺序、条件批次或远期储备时，再读 doc/milestone-timeline.md
6. doc/design/message-bus-design.md（先看 TL;DR）
7. backend/app/core/workflow_auto_advance.py
8. backend/app/core/runtime.py
9. backend/app/contracts/commands.py
10. backend/app/core/output_schemas.py

推进规则：
- 只做当前批次内可直接开启的 CEO 相关任务；`C1` 条件批次只有在触发条件成立时才可开启
- 当前 CEO 真实执行集必须始终按 `CREATE_TICKET / RETRY_TICKET / HIRE_EMPLOYEE / REQUEST_MEETING` 书写；`ESCALATE_TO_BOARD` 仍是 `DEFERRED_SHADOW_ONLY`
- CEO 输出动作必须经过 Reducer 级校验
- LLM 不可用时必须回退到当前确定性路径
- 不修改已工作的 Ticket / Approval / Employee handler，除非明确属于当前切片
- 不因远期愿景处于未开始状态就自动打开新能力
...
实现要求：
- 必须交付可运行代码 + 最小测试 + 必要文档更新
- 设计与代码不一致时，先说明差异，做最保守处理
- 不把 shadow only / deferred 能力写成已完成

--- 收尾（代码完成后自动执行以下全部步骤）---

第一步：验证
- 运行后端测试：cd backend && ./.venv/bin/pytest tests/ -q
- 如果本轮改到前端 CEO 相关读面，再运行：cd frontend && npm run build && npm run test:run
- 如果任何一项失败，先修复再继续
...
7. backend/app/core/provider_openai_compat.py
8. backend/app/core/context_compiler.py（需要时先看设计文档 TL;DR）
9. backend/app/core/output_schemas.py

推进规则：
- 只做当前批次内可直接开启的 Worker 相关任务；`C1` 条件批次只有在触发条件成立时才可开启
- 确定性路径必须保留为回退
- 真实 LLM 输出必须经过 schema 校验 + 写集验证
- Provider 错误必须分类并记录为 incident 事件
- 每个新能力都要有 mock LLM 响应的单元测试
- 不因远期愿景处于未开始状态就自动打开新系统能力
...
第一步：验证
- 运行后端测试：cd backend && ./.venv/bin/pytest tests/ -q
- 如果本轮改到前端 Worker 相关读面，再运行：cd frontend && npm run build && npm run test:run
- 如果任何一项失败，先修复再继续
...
5. 需要后续顺序、条件批次或远期储备时，再读 doc/milestone-timeline.md
6. git status 和 git diff --stat 了解当前变更
