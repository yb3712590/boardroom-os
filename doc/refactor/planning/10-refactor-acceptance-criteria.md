...
- [ ] 能定位并重放关键 provider failure。
  - Round 11B 部分证据：`provider-failure-015-malformed-sse` 已定位并重放，event range `9969..10016`，失败票 `tkt_0c616378c9ac`，核心事件 `9995 PROVIDER_ATTEMPT_FINISHED` / `9996 TICKET_FAILED`，attempt ref `attempt:wf_7f2902f3c8c6:tkt_0c616378c9ac:prov_openai_compat_truerealbill:1`。真实 015 旧数据将 malformed SSE 记录为 `PROVIDER_BAD_RESPONSE` 且没有 `raw_archive_ref`，`ReplayCaseResult.status=FAILED`，`issue_classification=replay/import issue`，diagnostics 为 `provider_raw_archive_ref_missing` / `provider_failure_taxonomy_legacy_bad_response`。因此本 checkbox 暂不勾，直到真实 replay 或新 live run 提供完整 raw archive refs。
  - Round 11B 已验证的边界：provider provenance 完整（preferred/actual provider `prov_openai_compat_truerealbill`，preferred/actual model `gpt-5.5`）；retry/recovery outcome 为 `retried_and_completed`，只消费结构化 failure detail、ticket terminal、retry 和 incident recovery event；late guard 通过，current ticket 为 `tkt_2b58304dccb9`，`raw_transcript_used=false`、`late_output_body_used=false`、`timestamp_pointer_guess_used=false`。
- [x] 能重放 BR-032 auth contract mismatch。Round 11C 证据：`contract-gap-015-br032-auth-mismatch`，event range `5855..6299`，case result path `D:/Projects/boardroom-os/.pytest-tmp/replay-contract-gap-015/br032.json`；checker report `art://runtime/tkt_bc0404503ec8/delivery-check-report.json` 记录 `BR032-F06`，replay 编译为 blocking `acceptance_missing_required_evidence`；`APPROVED_WITH_NOTES` 不放行，`graph_terminal_override_used=false`，rework target 指向 `tkt_c247833b2c60` / `node_backlog_followup_br_031_m3_frontend_auth_nav`。
- [x] 能阻断 BR-040/BR-041 placeholder delivery。Round 11C 证据：`contract-gap-015-br040-placeholder-delivery` (`11400..11464`) 阻断 `art://workspace/tkt_2252a7a1f92e/source.py`、`art://workspace/tkt_2252a7a1f92e/test-report.json`、`pa://source-code-delivery/tkt_2252a7a1f92e@1`、`pa://evidence-pack/tkt_2252a7a1f92e@1`，target 为 `tkt_2252a7a1f92e`；`contract-gap-015-br041-placeholder-delivery` (`9933..10016`) 阻断 `art://workspace/tkt_5707c310bc6d/source.py`、`art://workspace/tkt_5707c310bc6d/test-report.json`、`pa://source-code-delivery/tkt_5707c310bc6d@1`、`pa://evidence-pack/tkt_5707c310bc6d@1`，target 为 `tkt_5707c310bc6d`。两条 case 均记录 `placeholder_delivery_blocked`、`APPROVED_WITH_NOTES` 不放行、`graph_terminal_override_used=false`。
- [x] 能处理 orphan pending 不阻断 graph complete。Round 11D 证据：`graph-progression-015-orphan-pending-br060`，event range `11791..11903`，case result path `D:/Projects/boardroom-os/.pytest-tmp/replay-graph-progression-015/orphan-pending.json`；`tkt_262f159fc931` 不进入 ready/current/effective complete，current pointer 为 `tkt_5d8e536a14c2` 和 review `tkt_665d647e556a`，policy reason code `progression.stale_orphan_pending_ignored`，source graph version `gv_11903`，affected refs `["tkt_262f159fc931"]`。
  - Round 11D 额外 graph/progression 证据：CANCELLED/SUPERSEDED case `graph-progression-015-superseded-cancelled-br032` (`5960..6299`) 证明 `tkt_ade5951f10ec`、`tkt_2262491ff9ae`、`tkt_e2b36aef19e9` 不进入 effective edges/readiness/current/completed refs；late provider case `graph-progression-015-late-provider-current-pointer-br041` (`9969..10016`) 证明 current pointer 保持 `tkt_2b58304dccb9` 且 `timestamp_pointer_guess_used=false`；missing explicit pointer case `graph-progression-015-missing-explicit-pointer` (`11791..11903`) 证明缺 explicit pointer 时输出 `graph.current_pointer.missing_explicit` / `progression.incident.graph_reduction_issue`，不按 `updated_at` 猜 current。
- [ ] 能生成 closeout 但必须满足 deliverable contract。
  - Round 11E fail-closed 证据：`closeout-015-manual-m103-contract-gate`，event range `15778..15801`，case result path `D:/Projects/boardroom-os/.pytest-tmp/replay-closeout-015/closeout.json`。`tkt_4624a870959f` 拒绝 `art://project-workspace/wf_7f2902f3c8c6/10-project/ARCHITECTURE.md`，`tkt_737cd07e76e5` 拒绝 `art://runtime/tkt_f58cc1d4ab7b/backlog_recommendation.json`；最终 `tkt_7a888035b4ff` 是 manual M103 closeout，payload 缺 `deliverable_contract_version`、`deliverable_contract_id`、`evaluation_fingerprint` 和 `final_evidence_table`。
  - Contract summary：`status=BLOCKED`，reason `closeout_missing_final_evidence_table`，compiled final evidence rows `2`，submitted rows `0`；`manual_closeout_recovery_bypassed_contract=false`、`graph_terminal_override_used=false`、`checker_verdict_only_allowed=false`、`failed_delivery_report_only_allowed=false`、`governance_docs_only_allowed=false`。
  - 本 checkbox 暂不勾：Round 11E 证明旧 manual M103 closeout 不能绕过 contract，classification 为 `replay/import issue`，但没有证明真实 015 closeout 满足 contract/final evidence table。
- [x] 输出新的 replay audit report。Round 11E 证据：report path `D:/Projects/boardroom-os/.pytest-tmp/replay-audit-015/replay-audit-report.json`，report version `replay-audit-report.v1`，report hash `42765a88f830a1c3b03a489b10816089a9cdb69f3e1b1dfc07b8a227a7ebe8ec`，source manifest hash `8438fb6aed8e2daa32e90fd19ed171cb1691f06ebe5f0194e20f9e33ccda9d53`。报告逐项列出 015 import `READY`、provider failure `FAILED`、BR-032 `READY`、BR-040/BR-041 `READY`、orphan pending `READY`、contract closeout `FAILED`、audit report `READY`；issue taxonomy 固定为 `provider failure`、`runtime bug`、`product defect`、`contract gap`、`replay/import issue`，且 provider/closeout 旧数据问题保持 `issue_type=replay/import issue`，不伪装成 provider/runtime/product/contract issue。验证：`pytest --basetemp="D:/Projects/boardroom-os/.pytest-tmp" backend/tests/test_replay_closeout.py backend/tests/test_replay_audit_report.py -q` -> `5 passed, 1 warning`；audit CLI smoke exit 0。
