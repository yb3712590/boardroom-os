...
下一轮新会话应从 **Round 12：后端-only live scenario clean run** 开始。Round 9A 已完成版本化 DeliverableContract、DeliverableEvaluation、ContractFinding、最小 compiler 和纯 evaluator skeleton；Round 9B 已完成 required source surfaces path/capability/evidence 映射、EvidencePack 到 acceptance criteria 映射，以及 superseded/placeholder/archive/unknown/stale pointer evidence 的 fail-closed evaluator；Round 9C 已把 checker verdict gate 改为先消费 DeliverableEvaluation，并要求 failed delivery report 只能由结构化 ConvergencePolicy 明确放行；Round 9D 已把 blocking contract gap 编译为 Phase 4 recovery input，并将 rework target 指向 upstream source surface / producer ticket / producer node，缺 current producer ticket 时输出 incident/action；Round 9E 已把 closeout package 放行收口到 contract version/id、evaluation fingerprint 和 final evidence table。Round 10A 已建立 `ReplayResumeRequest` / `ReplayWatermark` / `ReplayResumeResult` 最小 contract、event id cursor 恢复入口、稳定 watermark hash 和缺 cursor / 越界 / projection version 不匹配 / event range 不连续的 fail-closed 诊断。Round 10B 已在同一 contract 上增加 graph version resume，`gv_<event sequence_no>` 映射到明确 event range / projection version / replay watermark，并重放事件重算 effective graph pointer、ready/blocked/complete index；graph version 缺失、断层、graph patch hash 不匹配都会 fail-closed。Round 10C 已在同一 contract 上增加 ticket id / incident id resume，默认恢复到最新 event 或显式 event cursor，并保留 ticket terminal/in-flight、runtime node view、assignment/lease、artifact/evidence/process asset refs、incident status、source ticket context、followup action 和 recovery action lineage；缺 ticket、缺 incident、source ticket 不一致、source ticket 缺失、runtime node view 断裂都会 fail-closed。Round 10D 已定义 `ReplayCheckpoint`，保存 projection payload、event watermark、schema/version/hash、covered projections 和 compatibility；有效 checkpoint 从 watermark 后增量 replay，无效 checkpoint 默认 fail-closed，显式 full replay fallback 会写 diagnostic；schema/version/watermark/hash/projection set/compatibility mismatch 均有测试覆盖。Round 10E 已定义 `ReplayHashManifest` / `ReplayBundleReport`，可验证 artifact index 与 materialized storage content hash 一致；缺 artifact、未登记 storage ref、hash mismatch 均 fail-closed。Round 10F 已定义 `boardroom-document-view.v1`，从 event/process asset/artifact metadata/content 重新物化 document view，并把 source event range、process asset refs、artifact refs、document refs、hash 和 diagnostics 接入 replay bundle/report；缺 artifact、非法 process asset、非法 evidence lineage 均 fail-closed。Round 11A 已建立 `ReplayImportManifest` 和 read-only import harness，015 manifest 为 `READY`，event range `1..15801`，manifest hash `8438fb6aed8e2daa32e90fd19ed171cb1691f06ebe5f0194e20f9e33ccda9d53`，idempotency key `replay-import:3a209b15acc022ff85a35f27efa4f5857f3151d51aaaf391dd7408cf866332b8`；导入诊断包括 archive metadata、未登记文件、mutable storage relpath、`INLINE_DB` artifact 和 source projection mismatch，均归类为 replay/import issue。Round 11B 已新增 `ReplayCaseResult` 与只读 provider replay harness，case `provider-failure-015-malformed-sse` 可定位到事件 `9969..10016`，失败票 `tkt_0c616378c9ac`，attempt ref `attempt:wf_7f2902f3c8c6:tkt_0c616378c9ac:prov_openai_compat_truerealbill:1`；真实 015 旧数据把 malformed SSE 记录为 `PROVIDER_BAD_RESPONSE` 且没有 `raw_archive_ref`，case result 归类为 `replay/import issue`，late guard 和 retry/recovery 只由结构化事件验证。Round 11C 已新增只读 contract gap replay harness，三条 case 均为 `contract gap replay evidence`：BR-032 `contract-gap-015-br032-auth-mismatch` (`5855..6299`) 保留 `BR032-F06` blocker，并将 rework target 指向 `tkt_c247833b2c60` / `node_backlog_followup_br_031_m3_frontend_auth_nav`；BR-040 `contract-gap-015-br040-placeholder-delivery` (`11400..11464`) 阻断 `tkt_2252a7a1f92e` 的 `source.py` / generic `pytest tests -q` evidence；BR-041 `contract-gap-015-br041-placeholder-delivery` (`9933..10016`) 阻断 `tkt_5707c310bc6d` 的 `source.py` / generic `pytest tests -q` evidence。Round 11D 已新增只读 graph/progression replay harness，四条 case 均为 `graph progression replay evidence`：orphan pending `graph-progression-015-orphan-pending-br060` (`11791..11903`) 证明 `tkt_262f159fc931` 不阻断 graph complete；CANCELLED/SUPERSEDED `graph-progression-015-superseded-cancelled-br032` (`5960..6299`) 证明 `tkt_ade5951f10ec`、`tkt_2262491ff9ae`、`tkt_e2b36aef19e9` 不进入 effective edges/readiness/current/completed refs；late provider `graph-progression-015-late-provider-current-pointer-br041` (`9969..10016`) 证明 current pointer 保持 `tkt_2b58304dccb9`；missing explicit pointer case 证明缺 explicit runtime pointer 时输出 `graph.current_pointer.missing_explicit` / `progression.incident.graph_reduction_issue`，不按 `updated_at` 猜 current。Round 11E 已新增只读 closeout replay harness 和 replay audit report，case `closeout-015-manual-m103-contract-gate` (`15778..15801`) 证明 `tkt_4624a870959f` 拒绝 `ARCHITECTURE.md`、`tkt_737cd07e76e5` 拒绝 `backlog_recommendation.json`，最终 `tkt_7a888035b4ff` 是旧 manual M103 closeout，缺 `deliverable_contract_version`、`deliverable_contract_id`、`evaluation_fingerprint` 和 `final_evidence_table`，因此 status `FAILED`、classification `replay/import issue`，不能勾为 contract closeout 通过。最新 replay audit report 为 `D:/Projects/boardroom-os/.pytest-tmp/replay-audit-015/replay-audit-report.json`，hash `42765a88f830a1c3b03a489b10816089a9cdb69f3e1b1dfc07b8a227a7ebe8ec`；Phase 7 中 015 import、BR-032、BR-040/BR-041、orphan pending、audit report 有 replay 证据，provider failure 和 contract closeout 保持未完成。后续不得重新设计 9A/9B/9C/9D/9E 的 contract id/finding id/evaluation fingerprint、source surface、evidence vocabulary、convergence policy、rework target、final evidence table 或 Phase 4 graph pointer/effective edge 语义，也不得把 checker verdict、graph terminal、checker notes freeform 文本、manual closeout recovery、closeout final refs、provider raw transcript、late output 正文、stale snapshot 或人工 projection repair 主路径当成 contract satisfaction。
...
Round 10A implementation status:

- `ReplayResumeRequest` / `ReplayWatermark` / `ReplayResumeResult` 已落在 `backend/app/contracts/replay.py`，统一使用 `replay-resume.v1` contract version 和 `SCHEMA_VERSION`。
- `backend/app/core/replay_resume.py` 提供 `resume_replay_from_event_id()` 纯函数入口。恢复点包含 event cursor、projection version、包含式 event range 和 replay watermark。
- Watermark hash 使用规范化 JSON 与 event log hash；相同 event log + same resume request 输出稳定。
- 缺 event cursor、cursor 越界、projection version 不匹配、event range 不连续都会 fail-closed 并返回 diagnostic。正常路径不调用 projection repair。
- 10B 必须复用 10A 的 request/watermark/result 语义，把 graph version 映射到明确 event range，不得另建不兼容恢复点。
...
Round 10C implementation status:

- `ReplayResumeRequest` 增加 `ticket_id` 和 `incident_id`；`ReplayWatermark` / `ReplayResumeResult` 继续复用 10A/10B 的 event cursor、projection version、event range、request hash、watermark hash 和 diagnostic 格式。
- `backend/app/core/replay_resume.py` 提供 `resume_replay_from_ticket_id()` / `resume_replay_from_incident_id()`。两者默认恢复到最新 event，也支持显式 `event_cursor`，`projection_version` 必须等于恢复边界 event sequence。
- Ticket resume 从 event log 重建临时 projection 后输出 terminal/in-flight 状态、runtime node view、assignment/lease context、current graph pointer，以及 artifact/evidence/process asset refs。in-flight ticket 缺 assignment/lease 或 runtime node view 断裂会 fail-closed。
- Incident resume 从 event log 重建临时 projection 后输出 incident status、source ticket context、recommended/final followup action、incident event lineage、recovery action lineage，以及 Phase 4 `recovery.actions` 形状的 rework/restore policy input。
- 缺 ticket、缺 incident、incident 与 pinned source ticket 不一致、source ticket 缺失、runtime node view 断裂都会 fail-closed；不通过手写 ticket/incident/runtime projection row 恢复上下文。
- 10D 必须继承四类 resume 的同一 contract 与 watermark 语义，再开始 Projection checkpoint；不得把 checkpoint 用作掩盖 10A–10C replay/projection bug 的 fallback。
...
Round 10E implementation status:

- `ReplayHashManifest` / `ReplayBundleReport` 已落在 `backend/app/contracts/replay.py`；字段覆盖 source event range、checkpoint refs/watermark、resume source、projection version、artifact refs、storage refs、content hash、materialization status、diagnostics 和 replay compatibility。
