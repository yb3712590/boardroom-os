...
Round 11B 真实 015 replay 结论（2026-05-04）：`provider-failure-015-malformed-sse` 可定位到事件 `9969..10016`，核心失败事件为 `9995 PROVIDER_ATTEMPT_FINISHED` 和 `9996 TICKET_FAILED`，失败票为 `tkt_0c616378c9ac`。真实旧数据中的 failure message 表明 provider stream emitted malformed SSE JSON，但 failure taxonomy 仍是旧 `PROVIDER_BAD_RESPONSE`，且 `failure_detail` / provider attempt event 都没有 `raw_archive_ref`。因此 `ReplayCaseResult.status=FAILED`，`issue_classification=replay/import issue`，diagnostics 为 `provider_raw_archive_ref_missing` 和 `provider_failure_taxonomy_legacy_bad_response`。这只能证明 015 旧数据缺 raw archive 证据，不能证明 malformed raw archive contract 在真实 replay 上通过。
...
Round 11B replay 对齐：旧票 `tkt_30c7a10979ae` 在 `9967 TICKET_FAILED` 后仍有 late provider events（`9974`、`9975`、`9978`、`9979`、`9994`），但 case range 结束时结构化完成票是 `tkt_2b58304dccb9`（`10016 TICKET_COMPLETED`）。11B harness 不读取 provider raw transcript、late output 正文或最终 source projection 来猜 current pointer；它只用 case range 内 terminal/retry/completed 事件判断 late guard。
...
Round 11B provider provenance replay：`tkt_0c616378c9ac` 的 ticket failure detail 与 provider attempt event 同时记录 `preferred_provider_id=prov_openai_compat_truerealbill`、`actual_provider_id=prov_openai_compat_truerealbill`、`preferred_model=gpt-5.5`、`actual_model=gpt-5.5`，provider policy ref 为 `provider-policy:prov_openai_compat_truerealbill:gpt-5.5:xhigh:openai_compat`。该证据覆盖 provenance 字段存在性，但不覆盖 raw archive contract 成功。
...
- Round 11B：真实 015 malformed SSE provider failure 已可定位和重放，但旧 replay 数据缺 `raw_archive_ref` 且 taxonomy 仍是 `PROVIDER_BAD_RESPONSE`；真实 raw archive 证据仍依赖后续 replay 数据或新 live run，Round 11C 不得复用 provider raw transcript 或 late output 正文弥补 contract gap。
