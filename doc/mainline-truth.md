...
- 当 autopilot workflow 的硬约束显式提到 `architect_primary` 时，实现 fanout 现在有第一段 architect gate：没有 active architect 时，controller 会先建议 `HIRE_EMPLOYEE`；architect 已在岗但还没有过 governance gate 的 `architect_primary` 文档时，会 fail-close，阻止实现票继续 fanout
