...
- `POST /api/v1/commands/incident-resolve` now provides a controlled manual recovery path: recovery first closes the breaker and moves the incident into `RECOVERING` instead of closing it immediately
- an explicit `RESTORE_AND_RETRY_LATEST_TIMEOUT` follow-up can reopen the node and create one bounded retry for the latest timeout ticket in the same transaction
...
- a recovering incident now auto-closes with `INCIDENT_CLOSED` once its follow-up retry eventually completes successfully
...
- `ticket-result-submit` is the primary structured-result governance entrypoint
- `ticket-complete` and `ticket-fail` remain available, but now as compatibility paths rather than the preferred unified entrypoint
...
There is still one known setup reality in this repository: `pip install -e .[dev]` may fail in a fresh environment because `setuptools` discovers both `app` and `data` in the flat `backend/` layout. This round did not change that packaging issue, so the commands below only document the runtime and test flow that is already verified in the current repo checkout.
