...
def build_ceo_shadow_system_prompt(snapshot: dict) -> str:
    ...
    return (
        ...
        "If snapshot.replan_focus.capability_plan.required_governance_ticket_plan exists, only CREATE_TICKET that exact governance ticket_payload before implementation fanout resumes.\n"
        "If snapshot.replan_focus.capability_plan.followup_ticket_plans exists, keep CREATE_TICKET proposals aligned with those planned ticket_payload values.\n"
        "When capability_plan provides ticket_payload, copy that ticket_payload shape exactly. Do not re-invent legacy fields or alternate execution_contract shapes.\n"
