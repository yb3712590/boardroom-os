...
def _build_runtime_success_payload(
        ...
        ticket_specs = [
            {
                ...
                "name": "终端风单页 UI 壳层",
                "priority": "P0",
                "scope": ["深色背景", "单色文本", "高信息密度", "terminal console 布局"],
            },
            {
                "ticket_id": "BR-T02",
                "name": "books 单表与数据库模型",
                "priority": "P0",
                "scope": ["books", "数据库", "id", "title", "author", "status", "IN_LIBRARY", "CHECKED_OUT"],
            },
            {
                "ticket_id": "BR-T03",
                "name": "后端 Add / Remove 接口",
                "priority": "P1",
                "scope": ["新增图书", "下架图书", "最小 API", "匿名终端模式"],
            },
            {
                "ticket_id": "BR-T04",
                "name": "后端 Check Out / Return 状态机",
                "priority": "P1",
                "scope": ["Check Out", "Return", "状态切换", "只改 status", "不记录借阅人"],
            },
            {
                "ticket_id": "BR-T05",
                "name": "馆藏列表与可借状态展示",
                "priority": "P1",
                "scope": ["纯文本列表", "新增输入框", "在馆过滤", "借出过滤", "books terminal"],
            },
            {
                "ticket_id": "BR-T06",
                "name": "最小回归与交付证据",
                "priority": "P1",
                "scope": ["状态切换验证", "列表展示回归", "source delivery", "closeout 证据"],
            },
        ]
    ...
    def _default_backlog_dependency_graph() -> list[dict[str, Any]]:
        return [
            {"ticket_id": "BR-T01", "depends_on": [], "reason": "先把终端风 UI 壳层定住。"},
            {"ticket_id": "BR-T02", "depends_on": [], "reason": "books 单表和状态枚举是后续所有动作的真相源。"},
            {"ticket_id": "BR-T03", "depends_on": ["BR-T02"], "reason": "新增和下架动作必须建立在 books 单表之上。"},
            {"ticket_id": "BR-T04", "depends_on": ["BR-T02", "BR-T03"], "reason": "借出归还状态机要复用已有 books 记录。"},
            {"ticket_id": "BR-T05", "depends_on": ["BR-T01", "BR-T02", "BR-T04"], "reason": "列表展示要消费终端壳层和最新状态切换结果。"},
            {"ticket_id": "BR-T06", "depends_on": ["BR-T03", "BR-T04", "BR-T05"], "reason": "回归和 closeout 等前面最小链路跑通后再收口。"},
        ]
        ...
        return [
            {
                ...
                "label": "推荐工单拆分",
                "summary": "把目标收敛成极简书本流转终端的最小实现链。",
                "content_markdown": "只做终端风 UI、books 单表、Add / Check Out / Return / Remove、纯文本列表和最小验证，不扩展 Auth、预约、罚金、报表或运维壳层。",
            },
            {
                "section_id": "dependency_and_sequence_plan",
                "label": "依赖关系与实施顺序",
                "summary": "先定终端壳层和 books 真相，再补状态切换、展示和回归。",
                "content_markdown": "BR-T01 和 BR-T02 并行起步；随后落 Add / Remove、Check Out / Return；最后用馆藏列表和最小回归把 closeout 证据收口。",
            },
        ]
