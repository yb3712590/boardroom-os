...
from app.core.replay_contract_gap import (
    ...
    failed_contract_gap_case_result,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Replay a contract gap case from an imported replay DB.")
    ...
    if not manifest_path.is_file():
        diagnostics.append(
            _diagnostic(
                "manifest_missing",
                "Replay contract gap requires an existing ReplayImportManifest JSON file.",
    ...
    if diagnostics:
        result = failed_contract_gap_case_result(
            case_id=case_id,
            manifest=manifest,
