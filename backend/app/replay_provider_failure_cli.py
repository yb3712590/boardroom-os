...
from app.core.replay_provider_failure import (
    REPLAY_IMPORT_ISSUE_CLASSIFICATION,
    failed_provider_failure_case_result,
    replay_provider_failure_case,
)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Replay a provider failure case from an imported replay DB.")
    ...
    if not manifest_path.is_file():
        diagnostics.append(
            _diagnostic(
                "manifest_missing",
                "Replay provider failure requires an existing ReplayImportManifest JSON file.",
    ...
    if diagnostics:
        result = failed_provider_failure_case_result(
            case_id=case_id,
            manifest=manifest,
