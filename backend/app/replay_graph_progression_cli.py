...
from app.core.replay_graph_progression import (
    ...
    failed_graph_progression_case_result,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Replay a graph/progression case from an imported replay DB.")
    ...
    if not manifest_path.is_file():
        diagnostics.append(
            _diagnostic(
                "manifest_missing",
                "Replay graph progression requires an existing ReplayImportManifest JSON file.",
    ...
    if diagnostics:
        result = failed_graph_progression_case_result(
            case_id=case_id,
            manifest=manifest,
