...
from app.core.output_schemas import ARCHITECTURE_BRIEF_SCHEMA_REF, SOURCE_CODE_DELIVERY_SCHEMA_REF
...
def test_required_hook_gate_returns_structured_not_applicable_for_governance_ticket(client) -> None:
    init_response = client.post(
        "/api/v1/commands/project-init",
        json=_project_init_payload("Governance ticket should stay out of required hook gate scope."),
    )
    workflow_id = init_response.json()["causation_hint"].split(":", 1)[1]
    ticket_id = "tkt_gov_hook_scope_001"
    node_id = "node_gov_hook_scope_001"
